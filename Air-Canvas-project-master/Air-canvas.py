...
# Default trackbar callback function (does nothing)
def setValues(x):
    pass

# Create trackbars to set HSV range for marker detection
...
while True:
    ...
    if len(cnts) > 0:
        ...
        if center[1] <= 65:  # Check toolbar buttons
            if 40 <= center[0] <= 140:
                bpoints = [deque(maxlen=1024)]
                gpoints = [deque(maxlen=1024)]
                rpoints = [deque(maxlen=1024)]
